...
| `thread/compacted` | `ContextCompacted` | `v2::ContextCompactedNotification` | `%Codex.Events.TurnCompaction{stage: :completed}` | Legacy mapping note: map `thread/compacted` (`codex/codex-rs/app-server-protocol/src/protocol/v2.rs:1706-1709`) → `TurnCompaction` with empty `compaction` |
