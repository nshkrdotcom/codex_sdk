...
- `examples/live_subagent_host_controls.exs` — one parent -> one child subagent workflow that explicitly enables the current runtime's `features.multi_agent` flag, sets thread/depth limits, streams spawn/wait events, exercises the full `Codex.Subagents` helper surface, and runs a direct child follow-up turn
