defmodule Codex.Runtime.Exec.Profile do
  ...
  def render_args(opts) when is_list(opts) do
    ...
    |> Shared.maybe_add_flag("--ephemeral", Keyword.get(opts, :ephemeral))
    |> Shared.maybe_add_flag("--ignore-user-config", Keyword.get(opts, :ignore_user_config))
    |> Shared.maybe_add_flag("--ignore-rules", Keyword.get(opts, :ignore_rules))
